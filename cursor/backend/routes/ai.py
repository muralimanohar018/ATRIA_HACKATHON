...
from ..ai_engine.email_ai import (
    generate_job_email, generate_contact_email, generate_shortlist_email, generate_welcome_email
)
...
def generate_resume_pdf():
    """Generate AI-enhanced resume PDF using templates, return PDF file, and send via email."""
    from flask import send_file
    data = request.get_json(force=True)
    resume_data = data.get("resume_data")
    template = data.get("template", "ats-modern")
    send_email_flag = data.get("send_email", False)  # Optional flag to send email
    recipient_email = data.get("email", "")  # Email to send resume to
    ...
    try:
        ...
        name = formatted_resume.get("header", {}).get("name", "User")
        email = recipient_email or formatted_resume.get("header", {}).get("email", "")
        filename = f"{name.replace(' ', '_')}_{template}_resume.pdf"
        
        # Step 4: Send email with PDF attachment if requested
        email_sent = False
        if send_email_flag and email:
            ...
            email_body = generate_resume_email(name, filename)
            
            # Read PDF bytes for attachment
            pdf_bytes = pdf_buffer.read()
            pdf_buffer.seek(0)  # Reset for download
            
            # Send email with PDF attachment
            email_sent = send_email(
                to=email,
                subject=f"Your Resume - {Config.COMPANY_NAME}",
                body=email_body,
