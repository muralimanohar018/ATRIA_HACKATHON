...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.code === 'ECONNABORTED') {
      ...
      return Promise.reject(new Error('Request timeout. Please check if backend is running.'))
    }
    if (error.message === 'Network Error' || !error.response) {
      console.error('Network error:', error.config?.url)
      return Promise.reject(new Error('Network error. Please check if backend is running on port 5001.'))
    }
    return Promise.reject(error)
  }
)
...
export const apiRoutes = {
  ...
  ai: {
    ...
    generateResumePDF: (resumeData: any, template: string, sendEmail?: boolean, email?: string) =>
      api.post("/ai/generate-resume-pdf", { 
