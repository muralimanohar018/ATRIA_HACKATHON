...
export default function ResumeBuilder() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const response = await apiRoutes.ai.uploadResume(file)
      if (response.data.success) {
        ...
        if (fields) {
          if (fields.name) setResumeData({ ...resumeData, personalInfo: { ...resumeData.personalInfo, name: fields.name } })
          if (fields.email) setResumeData({ ...resumeData, personalInfo: { ...resumeData.personalInfo, email: fields.email } })
          if (fields.phone) setResumeData({ ...resumeData, personalInfo: { ...resumeData.personalInfo, phone: fields.phone } })
          if (fields.skills) setResumeData({ ...resumeData, skills: fields.skills })
        }
      }
    } catch (error: any) {
  ...
  const downloadResume = async () => {
    ...
    try {
      ...
      const sendEmail = userEmail.length > 0
      
      // Generate AI-enhanced PDF resume from backend with email option
      const response = await apiRoutes.ai.generateResumePDF(
        resumeData, 
        template,
        sendEmail,  // send_email flag
